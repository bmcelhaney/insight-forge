...
- Two JSON export modes:
  - Data capture: same schema as `/api/analyze` (`/api/export/data/{nsn}`, `insight-forge.data-capture.v1` / v1.3). Atomic `unit_price` + `quantity`; one merchant-matched `links.url` per priced hit; `timings` + `url_coverage` for ops. Screenshots/Tigris parked.
...
- Clean APIs for external tools:
  - `POST /api/analyze` — **primary machine API** (Windmill): data-capture document only (`insight-forge.data-capture.v1` / v1.3). Ingest **pricing + `links.url`** only. Screenshots/Tigris parked.
