...
- Two JSON export modes:
  - Data capture: same schema as `/api/analyze` (`/api/export/data/{nsn}`, `insight-forge.data-capture.v1` / v1.3). Atomic `unit_price` + `quantity`; one primary `links.url` per hit; optional Tigris `proof.screenshot`.
...
- Clean APIs for external tools:
  - `POST /api/analyze` — **primary machine API**: data-capture document only (`insight-forge.data-capture.v1` / v1.3); focus on pricing hits + reliable product URLs (screenshots/Tigris currently disabled)
